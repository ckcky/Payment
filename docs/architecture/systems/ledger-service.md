...
下图由 `deployment/tools/gen-module-diagrams.py` 扫描 `ledger-service/src/main/java` **自动生成**，反映真实的包与类分布（DTO 不计入）。
...
```mermaid
flowchart TB
    INBOUND["入口层 · web / api（HTTP 入口与 DTO 转换）<br/>LedgerController"]
    APP["应用层 · application（用例编排）<br/>AccountDefinitionValidator · AccountResolver · BalanceChecker …"]
    DOMAIN["领域层 · domain（聚合根 / 状态机 / 不变式 / 仓储端口）<br/>AccountBalance · AccountBalanceRepository · AccountDefinition …<br/>posting：OriginalPostingLookup · PostingLine · PostingRule …<br/>rules：AdjustmentRule · ChannelFeeRule · ChannelSettlementRule …"]
    INFRA["基础设施层 · infra / persistence / mq / cache（实现端口与持久化）<br/>AccountBalanceEntity · AccountBalanceMapper · AccountDefinitionEntity …"]
    INBOUND --> APP
    APP --> DOMAIN
    DOMAIN -.->|端口实现| INFRA
    DB[("MySQL<br/>ledger 库")]
    INFRA --> DB
...
erDiagram
    account_definitions {
        ...
        varchar owner_dimension
        varchar status
        datetime created_at
    }
    account_balances {
        bigint account_instance_id PK
        varchar currency PK
        bigint debit_total
        bigint credit_total
        bigint entry_count
    ...
    postings ||--o{ ledger_entries : "posting_id（业务单号·无FK）"
    accounts ||--o{ ledger_entries : "account_id（业务单号·无FK）"
```
<!-- /diagram:er -->
...
```mermaid
flowchart LR
    APP["ledger-service<br/>应用层"]
    subgraph DB["MySQL 8.0 · localhost:3306 · ledger 库"]
        direction TB
        T_account_definitions["account_definitions"]
        T_account_balances["account_balances"]
        T_ledger_periods["ledger_periods"]
        T_accounts["accounts"]
        T_postings["postings"]
