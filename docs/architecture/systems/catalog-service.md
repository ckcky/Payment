...
下图由 `deployment/tools/gen-module-diagrams.py` 扫描 `catalog-service/src/main/java` **自动生成**，反映真实的包与类分布（DTO 不计入）。
...
```mermaid
flowchart TB
    INBOUND["入口层 · web / api（HTTP 入口与 DTO 转换）<br/>CatalogController · CatalogRedisConfig · SeckillStockController …"]
    APP["应用层 · application（用例编排）<br/>CatalogApplicationService · CatalogCacheProperties · StockApplicationService<br/>seckill：SeckillProperties · SeckillStockService"]
    DOMAIN["领域层 · domain（聚合根 / 状态机 / 不变式 / 仓储端口）<br/>Product · ProductRepository · ProductStatus …"]
    INFRA["基础设施层 · infra / persistence / mq / cache（实现端口与持久化）<br/>CatalogMqConfig · CatalogMqHandlers · InMemoryProductRepository …<br/>product：MybatisProductRepository · ProductEntity · ProductMapper<br/>sku：MybatisSkuRepository · SkuEntity · SkuMapper<br/>stock：MybatisStockRepository · MybatisStockReservationRepository · StockEntity …"]
    INBOUND --> APP
    APP --> DOMAIN
    DOMAIN -.->|端口实现| INFRA
    DB[("MySQL<br/>catalog 库")]
    INFRA --> DB
...
erDiagram
    ...
    products ||--o{ skus : "product_id（业务单号·无FK）"
    skus ||--|| stock : "sku_id（业务单号·无FK）"
    skus ||--o{ stock_reservation : "sku_id（业务单号·无FK）"
```
<!-- /diagram:er -->
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> SELLABLE : activate
    SELLABLE --> SUSPENDED : suspend
    SELLABLE --> DISCONTINUED : discontinue
...
```mermaid
flowchart LR
    APP["catalog-service<br/>应用层"]
    subgraph DB["MySQL 8.0 · localhost:3306 · catalog 库"]
        direction TB
        T_products["products"]
        T_skus["skus"]
        T_stock["stock"]
        T_stock_reservation["stock_reservation"]
    end
    APP -->|MyBatis| DB
