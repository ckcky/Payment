...
```mermaid
flowchart TB
    INBOUND["入口层 · web / api（HTTP 入口与 DTO 转换）<br/>InternalServiceAuthInterceptor · MockCashierProperties · PaymentController …<br/>channelgateway：ChannelAdminController · ChannelCallbackController · ChannelCallbackSignatureFilter …<br/>limit：LimitController ｜ posting：PendingPostingAdminController"]
