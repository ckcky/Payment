...
```mermaid
flowchart TB
    INBOUND["入口层 · web / api（HTTP 入口与 DTO 转换）<br/>AlipayNotifyController · ChannelAdminController · ChannelCallbackController …<br/>limit：LimitController ｜ posting：PendingPostingAdminController<br/>refund：RefundController · RefundFactsController"]
    APP["应用层 · application（用例编排）<br/>ChannelAttemptRecorders · CreatePaymentCommand · LedgerPostingGateway …<br/>channel：AlipayGateway · CallbackUrls · ChannelAttemptRecorder …<br/>limit：LimitCompensationScanner · LimitCompensationScheduler · LimitExpiryIndex …<br/>posting：PostingEventTypes · PostingPendingRecorder · PostingProperties …<br/>refund：CreateRefundCommand · LedgerPostingGateway · MockRefundResultBridge …<br/>reliability：ChannelQueryScheduler · ChannelQueryService · PaymentRetryService …<br/>spi：ChannelCallbackEnvelope · ChannelPluginDescriptor"]
    DOMAIN["领域层 · domain（聚合根 / 状态机 / 不变式 / 仓储端口）<br/>Payment · PaymentAttempt · PaymentAttemptErrorType …<br/>limit：LimitExceededException · LimitOperation · LimitOperationRepository …<br/>posting：PendingPosting · PendingPostingRepository<br/>refund：Refund · RefundDecision · RefundItem …"]
    INFRA["基础设施层 · infra / persistence / mq / cache（实现端口与持久化）<br/>AlipaySandboxProperties · ChannelAttemptRecorderImpl · FeignLedgerPostingGateway …<br/>alipay：AlipaySdkGateway<br/>attempt：AttemptExtraCodec · MybatisPaymentAttemptRepository · PaymentAttemptEntity …<br/>channel：AbstractMockChannelAdapter · AlipayChannelAdapter · ConfiguredChannelRouter …<br/>limit：LimitConfiguration · LimitOperationEntity · LimitOperationMapper …<br/>posting：MybatisPendingPostingRepository · PendingPostingEntity · PendingPostingMapper …<br/>refund：InMemoryRefundRepository · LedgerFeignClient · LocalPaymentRefundGateway …"]
    INBOUND --> APP
    APP --> DOMAIN
    DOMAIN -.->|端口实现| INFRA
