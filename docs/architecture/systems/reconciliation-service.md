...
下图由 `deployment/tools/gen-module-diagrams.py` 扫描 `reconciliation-service/src/main/java` **自动生成**，反映真实的包与类分布（DTO 不计入）。
...
```mermaid
flowchart TB
    INBOUND["入口层 · web / api（HTTP 入口与 DTO 转换）<br/>ReconciliationController · ReconciliationExcludedFact · ReconciliationSettlementFact<br/>audit：AuditController<br/>posting：PendingPostingAdminController · PostingAdminGuardInterceptor · PostingWebConfig"]
    APP["应用层 · application（用例编排）<br/>AutoDispositionService · ChannelFundPostingService · FundPostingGateway …<br/>audit：AuditApplicationService · AuditFactsGateway · AuditLedgerGateway …<br/>posting：PostingEventTypes · PostingPendingRecorder · PostingProperties …"]
    DOMAIN["领域层 · domain（聚合根 / 状态机 / 不变式 / 仓储端口）<br/>AutoDispositionPolicy · ChannelFundFact · ChannelStatement …<br/>audit：AdjustmentPolicy · AuditAdjustment · AuditAdjustmentKind …<br/>posting：PendingPosting · PendingPostingRepository<br/>statement：StatementImport · StatementImportRepository · StatementImportStatus …"]
    INFRA["基础设施层 · infra / persistence / mq / cache（实现端口与持久化）<br/>CsvStatementParser · FactsClientConfig · FeignFundPostingGateway …<br/>audit：AuditAdjustmentEntity · AuditAdjustmentMapper · AuditBatchEntity …<br/>posting：MybatisPendingPostingRepository · PendingPostingEntity · PendingPostingMapper …"]
    INBOUND --> APP
    APP --> DOMAIN
    DOMAIN -.->|端口实现| INFRA
    DB[("MySQL<br/>reconciliation 库")]
    INFRA --> DB
    REDIS[("Redis<br/>缓存 / Redis Streams 事务消息")]
    INFRA --> REDIS
    FeignPaymentFactsClient["FeignPaymentFactsClient"]
    INFRA -.->|Feign RPC| FeignPaymentFactsClient
    FeignRefundFactsClient["FeignRefundFactsClient"]
    INFRA -.->|Feign RPC| FeignRefundFactsClient
    LedgerAuditFeignClient["LedgerAuditFeignClient"]
    INFRA -.->|Feign RPC| LedgerAuditFeignClient
    LedgerEventFeignClient["LedgerEventFeignClient"]
    INFRA -.->|Feign RPC| LedgerEventFeignClient
    PaymentFactsClient["PaymentFactsClient"]
    INFRA -.->|Feign RPC| PaymentFactsClient
    PaymentFactsFeignClient["PaymentFactsFeignClient"]
    INFRA -.->|Feign RPC| PaymentFactsFeignClient
    RefundFactsClient["RefundFactsClient"]
    INFRA -.->|Feign RPC| RefundFactsClient
    RefundFactsFeignClient["RefundFactsFeignClient"]
    INFRA -.->|Feign RPC| RefundFactsFeignClient
    SettlementAuditFeignClient["SettlementAuditFeignClient"]
    INFRA -.->|Feign RPC| SettlementAuditFeignClient
```
<!-- /diagram:module -->
...
erDiagram
    ...
    statement_lines {
        ...
        varchar channel_txn_no
        varchar reference_type
        varchar reference
        varchar reference_kind
        varchar merchant_id
        bigint amount_minor
        bigint fee_minor
        char currency
    ...
    reconciliation_differences {
        ...
        bigint actual_amount_minor
        bigint fee_amount_minor
        char currency
        varchar status
        varchar disposition_ref
    ...
    audit_differences {
        ...
        varchar currency
        varchar status
        bigint suspended_amount_minor
        bigint adjusted_amount_minor
        bigint transferred_out_minor
        varchar detail
        varchar resolution_note
        varchar resolved_by
        datetime resolved_at
...
stateDiagram-v2
    [*] --> PENDING
    PENDING --> RECONCILING : start
    RECONCILING --> CONSISTENT : finish
    CONSISTENT --> CLOSED : close
    RECONCILING --> HAS_DIFFERENCE : finish(有差异)
```
<!-- /diagram:state -->
...
```mermaid
flowchart LR
    APP["reconciliation-service<br/>应用层"]
    subgraph DB["MySQL 8.0 · localhost:3306 · reconciliation 库"]
        direction TB
        T_statement_imports["statement_imports"]
        T_statement_lines["statement_lines"]
        T_reconciliation_differences["reconciliation_differences"]
        T_pending_postings["pending_postings"]
        T_reconciliation_batches["reconciliation_batches"]
        T_audit_batches["audit_batches"]
        T_audit_differences["audit_differences"]
        T_audit_adjustments["audit_adjustments"]
    end
    APP -->|MyBatis| DB
