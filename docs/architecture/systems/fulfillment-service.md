...
下图由 `deployment/tools/gen-module-diagrams.py` 扫描 `fulfillment-service/src/main/java` **自动生成**，反映真实的包与类分布（DTO 不计入）。
...
```mermaid
flowchart TB
    INBOUND["入口层 · web / api（HTTP 入口与 DTO 转换）<br/>FulfillmentController · FulfillmentRefundController · PaymentSuccessRpcController"]
    APP["应用层 · application（用例编排）<br/>EntitlementGateway · FulfillmentApplicationService"]
    DOMAIN["领域层 · domain（聚合根 / 状态机 / 不变式 / 仓储端口）<br/>Fulfillment · FulfillmentRepository · FulfillmentStatus"]
    INFRA["基础设施层 · infra / persistence / mq / cache（实现端口与持久化）<br/>EntitlementFeignClient · FulfillmentEntity · FulfillmentEventPublisher …"]
    INBOUND --> APP
    APP --> DOMAIN
    DOMAIN -.->|端口实现| INFRA
    DB[("MySQL<br/>fulfillment 库")]
    INFRA --> DB
    REDIS[("Redis<br/>缓存 / Redis Streams 事务消息")]
    INFRA --> REDIS
    EntitlementFeignClient["EntitlementFeignClient"]
    INFRA -.->|Feign RPC| EntitlementFeignClient
```
<!-- /diagram:module -->
...
erDiagram
    fulfillments {
        bigint id PK
        varchar source_payment_no UK
        varchar order_item_id
        varchar order_no
        varchar delivery_content
        varchar status
...
```mermaid
flowchart LR
    APP["fulfillment-service<br/>应用层"]
    subgraph DB["MySQL 8.0 · localhost:3306 · fulfillment 库"]
        direction TB
        T_fulfillments["fulfillments"]
    end
    APP -->|MyBatis| DB
