...
- **⚠️ stage-05 设计状态（2026-09-21）：`031` / `032` / `033` / `034` / `035` 均为「🟡 设计完成、待 Architecture Review 与负责人裁决」，一律未实现**。本轮为 **design-only** 产物：五个 spec + 七条 ADR（[ADR-0077~0079](../adr/0077-ledger-accounting-foundation-decisions.md) / [0080](../adr/0080-reconciliation-statement-and-fund-facts.md) / [0081](../adr/0081-test-carrier-and-schema-replayability.md) / [0082](../adr/0082-failure-recovery-ownership-and-compensation.md) / [0083](../adr/0083-observability-baseline-slo-and-cardinality.md)，**全部 🟡 Proposed**）+ 跨 Feature 收口 [design-summary.md](../specs/stage-05-channel-and-finance-deepening/design-summary.md)；未改任何 Java 业务代码 / 业务逻辑 / migration / 测试 / 运行时依赖。**在 ADR 转 Accepted 之前，本节不得改写为 Implemented**（合计 27 项待人类裁决：031 的 D-1~D-7 + H-032-1~6 + H-033-1~5 + H-034-1~5 + H-035-1~4）。
