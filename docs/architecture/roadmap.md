...
- **`034-reliability-hardening` 已实现（2026-09-21，[ADR-0082](../adr/0082-failure-recovery-ownership-and-compensation.md) Accepted）**：退款收口同事务化（C-19：状态收口与记账事件/失败台账同事务，MQ 通知与审计保持事务外）+ 调用方 `pending_postings` 台账（payment/settlement/reconciliation 三库、10s 扫描 + 1s/5s/30s/2m/10m 退避、6 次失败转 ABANDONED、`POST /internal/postings/{id}/replay` 运维重放、`ledger_posting_pending` gauge）+ 退款未知态查询（RefundUnknownQueryScheduler，75s 窗口）与滞留退款单扫描（StrandedRefundOrderScanner，红线 ≤2、只告警不猜成败 R-3）+ 迟到成功回调 `late=true` 标记（C-23）+ MQ DLQ 读/回放/清空运维端点（common-redis-mq `dlq` 包，`payment.mq.dlq-admin.enabled` 默认 false）+ Resilience4j 移除（H-034-1 影子弹性裁决）+ 三项诊断修复（调度器新 trace 根、毒丸隔离、DemoProxy 日志降级）+ 超时政策表文档化（technical-solution §5.1.1，不改值）。`mvn verify` 930 tests 全绿（15 模块，EXIT=0）；spec 034 三件套（plan/tasks/acceptance）齐。
- **`032-reconciliation-real-statement` 已实现（2026-09-21，[ADR-0080](../adr/0080-reconciliation-statement-and-fund-facts.md) Accepted）**：渠道账单成为导入对象（`statement_imports`/`statement_lines`/`reconciliation_differences` 三新表 + 守卫式增量迁移，SHA-256 指纹三层幂等之导入层，v2/legacy 双表头 CSV 解析）+ 匹配键升级 `(merchantId, referenceType, reference)` 三级降级 typed 匹配（弱匹配只出候选）+ 差异台账拆表（RD 单号，`differences_json` 停写不停读）+ run 编排（无导入 400 `STATEMENT_UNAVAILABLE`，sample.csv 回退退役）+ 渠道资金事实入账（`CHANNEL_SETTLEMENT`/`CHANNEL_FEE` 唯一产生方，`CS-`/`CF-{channel}-{period}` 派生幂等，双计三防线）+ 结算口径「全部已确认事实 − excludedFacts 净影响」（关闭 C-13）+ confirmed-facts 期间过滤与商户锚（关闭 C-20/C-22）+ 处置策略化（`ADJUSTING`/`ADJUST_FAILED` 分态、AutoDisposition 默认双关、audit 人工收口端点关闭 F7 跨账差异）。demo audit/reconciliation 场景升级导入前置与按单号收口；T24b 已随合并 034 后回归收口（968+ tests 全绿 + 三场景实测）；spec 032 三件套（plan/tasks/acceptance）齐。
...
- **`041-payment-service-governance` 为 Approved（2026-09-26 负责人裁决批准立项，H-041-1~3 已裁决；**ADR-0084 为 Proposed，待 Accept，Accept 前不得动代码**）**：对整个 payment-service 实施 Payment/Channel 双域四层重构（`channelgateway`→`channel` 正名、`posting` 并入 `payment`、四个旁路包收进四层），统一以 `ChannelGateway`（Payment → Channel）与 `PaymentResultPort`（Channel → Payment）隔离交互；API 允许仓内一次性替换（禁止新旧混跑、不保留兼容层），开发/测试环境允许清库重建 Schema（`APP_ENV` fail-closed）。
  - **已完成**：T01（ADR-0084 草案 + ADR 索引/traceability/ADR-0072 双向 Supersede 登记）、T02（四件套 + specs 索引 + 实测回归基线 **1086 tests / 0F / 0E**）、T03（迁移映射清单，含 12 个 Controller、4 组路径变更、9 个 spec 未给出目标的端点缺口）。
  - **Supersedes ADR-0072 §6「分层 ≠ 拆事务」**（旧 041 `57e12c3` 已按两域不共享事务落地，属 Code ≠ ADR 的收口）；**须同步 ADR-0083 排除路径补 `/callbacks/**`**（新回调端点 `/callbacks/channels/{channelCode}` 脱离原 `/internal/channels/**` 排除范围，不补即重开渠道报文进日志的漏洞）。
  - 详见 `docs/specs/stage-05-channel-and-finance-deepening/041-payment-service-governance/`。
