...
- **`028-channel-routing` 已立项待实现（2026-09-16）**：payment-service 两层结构重构
  ...
  > ⚠️ 本项目此前的「已实现」清单以 `018`~`026` 为止；`027` 为历史缺口（有意保留），
  > 下一个已立项但未实现的 spec 是 `028`。
- **当前 Feature**：`028-channel-routing`（立项待实现，等待 ADR-0072/0073 核准）——
  详见 `docs/specs/028-channel-routing/spec.md`。
