...
| ADR-0084 | payment-service 双域治理（041）：Payment/Channel 对称四层 + `ChannelGateway`/`PaymentResultPort` 双向端口 + 一次性 API 替换 + 仅 dev/test 清库重建 Schema + 插件目录自包含 + ArchUnit 双域/四层/Controller/持久化门禁；**Supersedes ADR-0072 §6**；**强制同步 ADR-0083 排除路径补 `/callbacks/**`** | **（🟡 Proposed，L0 尚无落点）** spec 041 设计轮产物；**Accept 前不得进入 L0、不得进入实现**；落点待定：`docs/architecture/systems/payment-service.md`、`docs/architecture/technical-solution.md` §4.3、`docs/operations/runbook.md` |
