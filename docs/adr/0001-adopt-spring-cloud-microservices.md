...
> **口径注明（2026-09-22 文档治理，加法不减原文）**：第 3 条「当前不使用 MQ 或跨服务异步事件」是 **2026-08 的决策快照**；其**现行状态**已由 [ADR-0074](0074-redis-transactional-message.md#adr-0074) 增补（落地于宪法 §IV + Core Principle V.3 增补）：**仍不引入 MQ 中间件**，跨服务**异步解耦**改用**既有 Redis（`redis:7`）Streams 模拟 MQ** 承载事务消息，目的是减少需要新增与长期运维的组件。本条原文保留不改写。
