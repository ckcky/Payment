...
| [0084](0084-payment-channel-governance.md) | payment-service 双域治理：Payment/Channel 对称四层 + `ChannelGateway`/`PaymentResultPort` 双向端口 + 一次性 API 替换 + 仅开发/测试清库重建 Schema（ADR-0084） | 🟡 **Proposed**（2026-09-26 起草；待负责人 Accept，H-041-4~6 待裁决） | **[spec 041](../specs/stage-05-channel-and-finance-deepening/041-payment-service-governance/spec.md)**；`channelgateway` → `channel` 正名、`posting` 并入 `payment`、四个旁路包收进四层；**Supersedes ADR-0072 §6「分层 ≠ 拆事务」**（两域不共享事务，旧 041 `57e12c3` 已按此落地）；**强制同步 ADR-0083 排除路径补 `/callbacks/**`**（端点 `/internal/channels/{code}/callback` → `/callbacks/channels/{code}`，不补即重开渠道报文进日志的漏洞）；旧 041（`payment-flow-layering`）视为可替换基线；吸收 spec 040 |

## ADR 完整文件清单（43 文件，权威）

> 本表是 **ADR 编号 → 标题 → 当前状态 → 承载文件**的**权威登记处**，覆盖 `docs/adr/` 下全部 43 个 ADR 文件（不含本 README 与 [traceability.md](traceability.md)）。状态与各 ADR 文件正文一致；新增 / 变更 ADR 时 MUST 同步本表与 [traceability.md](traceability.md)。
...
| 0084 | payment-service 双域治理：Payment/Channel 对称四层 + 双向端口 + 一次性 API 替换 + 仅开发/测试清库重建（**Supersedes ADR-0072 §6**） | 🟡 Proposed（2026-09-26 起草；待负责人 Accept） | [0084](0084-payment-channel-governance.md) |
...
| [0080](0080-reconciliation-statement-and-fund-facts.md#adr-0080) | 对账真实化（032）：账单导入对象（`statement_imports`/`statement_lines`，删静默回退）+ 匹配键升级 `(merchantId, referenceType, reference)` 三级降级 + 差异复用 5 态生命周期并拆独立行表 + 032 为 `CHANNEL_SETTLEMENT`/`CHANNEL_FEE` 唯一产生方 + 结算口径先于差异策略化（🟢 Accepted，2026-09-21 按 spec 推荐方案批准 H-032-1~6；随 spec 032 实现落地） | 0080 |
...
| [0082](0082-failure-recovery-ownership-and-compensation.md#adr-0082) | 失败恢复归属与补偿边界（034）：恢复三不变式 R-1/R-2/R-3 + `pending_postings` 调用方侧一张表复用形状（5 次退避 ⇒ `ABANDONED`）+ C-19 后置动作与终态**同事务**（A 方案）+ **移除** Resilience4j + refund 侧对称补有界收敛（UNKNOWN 查询 / 卡 `REQUESTED` 扫描，自动重放 ≤2）+ 迟到成功复用 surplus 出口（🟢 Accepted 2026-09-21，H-034-1~5 按 spec 推荐批准，随 spec 034 实现落地） | 0082 |
| [0083](0083-observability-baseline-slo-and-cardinality.md#adr-0083) | 观测基线（035）：4 个 SLO 以 Recording Rule 表达（双窗 burn rate，目标值 ≠ 实测值）+ 高基数政策 HC-1~HC-4 与 `MeterRegistry` 断言 + 密钥/报文不入日志取**路径排除**（不推翻 ADR-0027）+ **保留自研 traceId、否决 APM** + 告警扩容至 24 条五要素齐全、既有零删改（🟢 Accepted 2026-09-21，H-035-1~4 按 spec 推荐批准，随 spec 035 实现落地） | 0083 |
| [0084](0084-payment-channel-governance.md#adr-0084) | payment-service 双域治理（041）：Payment/Channel 对称四层（`channelgateway`→`channel` 正名、`posting` 并入 `payment`）+ `ChannelGateway`/`PaymentResultPort` 双向端口 + 跨域 DTO 入 `common-dto` + 一次性替换 9 组 HTTP API（不保留兼容层）+ 仅 dev/test 清库重建 Schema（`APP_ENV` fail-closed）+ 插件目录自包含 + **Supersedes ADR-0072 §6「分层 ≠ 拆事务」** + ArchUnit 双域/四层/Controller/持久化门禁与阳性对照（🟡 Proposed，2026-09-26 起草，待负责人 Accept；H-041-1~3 已裁决批准，H-041-4~6 待裁决） | 0084 |
