...
| 0032（**待 spec 027 合并**） | 用户支付限额——日月年周期额度与两阶段预占（ADR-0071） | 🟡 **Proposed**（2026-09-16 提出；D1~D8 已确认，D9~D10 待确认） | spec 027；三表模型（`user_payment_limits` / `user_limit_usage` / `limit_operations`）+ 两阶段预占（RESERVE → CONFIRM / RELEASE）+ 三道闸门（状态机终态吸收 / 幂等流水 UK / 以 payment 为事实源的补偿扫描）；与 ADR-0028「最小风控」切割（风控 ⛔ 不做，限额属业务合规能力）；⚠️ **ADR 文件与本索引行随 spec 027 合并补入**（当前在 worktree `docs/spec-027-user-payment-limit`，未 merge） |
| [0033](0033-two-layer-channel-architecture.md) | payment-service 两层结构：payment 支付层 / channelAttempt 渠道层（ADR-0072） | 🟡 **Proposed**（2026-09-16 提出，待负责人确认） | spec 028；确立两层**各拥自己的聚合与表写入口**（`payments` 只由 payment 层写、`payment_attempts` 只由渠道层写）；`PaymentChannel` 补 `channelCode()` 渠道身份；渠道实现族＝抽象基类 + Alipay/Wechat/Douyin 三实现；**退款 attempt 渠道取自原始支付记录**（消除 `PaymentRefundService:89` 硬编码 `"mock"`）；**分层 ≠ 拆事务**（两层共享本地事务）；明确不做：拆微服务 / 拆数据源 / 拆事务边界 |
| [0034](0034-channel-routing.md) | 支付渠道路由：注册表 + 规则化确定性选路（ADR-0073） | 🟡 **Proposed**（2026-09-16 提出，待负责人确认） | spec 028；**依赖 ADR-0072 的渠道身份**（渠道没有身份，注册表没有 key 可挂）；`ChannelRegistry` / `ChannelRouter` 落 `application/channel` 层（守 `Payment ≠ Channel`）、`channelCode` 变可选且**显式优先**（保 INV-2）、**确定性选路**（禁随机/计数器，保幂等键稳定）、**前向选路与反向按记录解析严格分离**（退款/重试/查询禁止重新路由）；配置化 `enabled + priority` + 静态 `availability` 桩（不读 Resilience4j）；启动强校验不静默降级；**Supersedes spec 015 §8 第 1 条「不做注册表」**；明确不做：自动降级 failover（**与 INV-2 互斥**）/ 智能路由（无数据源）/ 多商户号 / 渠道真实探活 / 改枚举 |
...
| 0071（**待合并**） | 用户支付限额：日月年周期额度（daily / monthly / yearly 三档）+ 两阶段预占（RESERVE → CONFIRM / RELEASE）+ 三表模型 + 三道闸门；与 ADR-0028「最小风控」切割（🟡 Proposed，2026-09-16 提出，D1~D8 已确认） | 0032（待合并） |
...
- **下一可用编号：ADR-0074**（ADR-0070 = 本地全栈容器化，见 `0031-containerized-local-stack.md`；ADR-0071 = 用户支付限额，见 `0032-user-payment-limit.md`（**待 spec 027 合并**，当前在 worktree）；ADR-0072 = payment-service 两层结构，见 `0033-two-layer-channel-architecture.md`；ADR-0073 = 支付渠道路由，见 `0034-channel-routing.md`。⚠️ 0071 若 spec 027 被否决、0072/0073 若 spec 028 被否决或改号，此处水位随之回退）。
