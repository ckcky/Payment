...
│       └── ...
│
├── gateway/                     # 接入层：Spring Cloud Gateway
│
├── common/                      # 共享库（被服务依赖，不独立部署）
│   ├── common-core/             # 通用工具、异常、统一返回体、结果码
│   ├── common-dto/              # 跨服务 DTO / 事件定义
...
└── src/
    ...
    │   │   ├── infra/                        # 基础设施：Repository 实现、MyBatis mapper、外部 client
