...
- Feign 超时/熔断/重试策略统一（审计 M2）——原计划并入本 spec，涉及 4 套策略并存的行为变更，影响面大，**拆到后续独立 spec**（将立项超时/熔断/重试策略矩阵 ADR 后单独实施；**编号按届时水位取用**——2026-09-16 水位已到 ADR-0073：0070「026 容器化」、0071「027 用户支付限额」（worktree，未 merge）、0072/0073「028 两层结构 + 渠道路由」）。
