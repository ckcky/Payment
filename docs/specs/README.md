...
- [032-reconciliation-real-statement](stage-05-channel-and-finance-deepening/032-reconciliation-real-statement/) —— 🟡 **Spec v1.0（2026-09-21，本轮只出设计）**：渠道账单成为导入对象（`statement_imports`/`statement_lines`，**删除 `sample.csv` 静默回退**）+ 匹配键升级 `(merchantId, referenceType, reference)` 三级降级 + 差异拆独立行表并复用四核对 5 态 + 032 为 `CHANNEL_SETTLEMENT`/`CHANNEL_FEE` 唯一产生方 + 结算口径先于差异策略化；关闭 C-13/C-20/C-22；决策 [ADR-0080](../adr/0080-reconciliation-statement-and-fund-facts.md)（**Proposed，待裁决 H-032-1~6**）。
- [033-test-infrastructure](stage-05-channel-and-finance-deepening/033-test-infrastructure/) —— 🟡 **Spec v1.0（2026-09-21，本轮只出设计）**：Testcontainers 放宽至**仅测试作用域**（`deployment/test-infra` 共享基座，只切幂等竞争/并发累加/并发唯一三类）+ schema 双路径重放成 CI 门禁 + 迁移可重放条文与 lint + 运行时 RPC 边允许清单；决策 [ADR-0081](../adr/0081-test-carrier-and-schema-replayability.md)（**Proposed，待裁决 H-033-1~5；触及 Constitution §Engineering.3**）。
- [034-reliability-hardening](stage-05-channel-and-finance-deepening/034-reliability-hardening/) —— 🟡 **Spec v1.0（2026-09-21，本轮只出设计）**：恢复三不变式 R-1/R-2/R-3 + 失败分类 X-1~X-17 + `pending_postings` 调用方台账（031 §12 的首次实现归属）+ C-19 后置动作同事务收口 + refund 侧对称有界收敛 + DLQ 可读可 replay + **移除** Resilience4j；决策 [ADR-0082](../adr/0082-failure-recovery-ownership-and-compensation.md)（**Proposed，待裁决 H-034-1~5**）。
- [035-observability-slo](stage-05-channel-and-finance-deepening/035-observability-slo/) —— 🟡 **Spec v1.0（2026-09-21，本轮只出设计）**：指标目录（跨 Feature 单一登记处）+ 高基数政策 HC-1~HC-4 与 `MeterRegistry` 断言 + 4 个 SLO 以 Recording Rule 表达（双窗 burn rate，目标 ≠ 实测）+ 告警 7→20 条五要素 + 密钥/报文不入日志取**路径排除**（不推翻 ADR-0027）+ **否决引入 APM**；决策 [ADR-0083](../adr/0083-observability-baseline-slo-and-cardinality.md)（**Proposed，待裁决 H-035-1~4**）。
...
> ⚠️ **状态说明**：`031` 编号与范围已由负责人 2026-09-21 裁决（见 spec 031 §0.2）；**032~035 为「设计完成、待评审」**——**均未实现**，其 ADR 均为 🟡 Proposed，**未 Accepted 前不得进入 plan/implement**；**其余 Feature 仍为提案**。
