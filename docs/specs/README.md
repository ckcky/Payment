...
- [038-payment-service-package-boundary](stage-05-channel-and-finance-deepening/038-payment-service-package-boundary/) —— 🟢 **已实现（2026-09-25，spec + Plan/Tasks/Acceptance 四件套齐）**：payment-service 一级包由「payment / refund / posting」收敛为「**payment（资金动作域）/ channelgateway（渠道网关域）/ posting（跨切面）**」——消灭 `com.payment.refund` 顶层包（32 主 + 10 测试迁入 `com.payment.payment.**`）+ 40 个渠道件 100% 收拢 `com.payment.channelgateway.**` + 退款内部端点收口为 `/internal/payments/refunds/**`（⚠️ **破坏性变更**，6 文件 8 处调用方已同步）；`ServiceBoundaryTest` 新增 4 条包级门禁（含 1 条**覆盖修补**——渠道件搬出 `com.payment.payment..` 后会脱离既有跨服务门禁，且全部带阳性对照防空转），既有 4 处反向依赖**逐条 FQN 白名单化**为技术债 TD-4 / TD-5（收口归 037，**未静默放宽规则**）。**本 Feature 未新增 ADR**（纯结构重构，行为零变化）。旁注：`036` / `037` / `039` 尚未登记入本索引。
