...
- [041-payment-service-governance](stage-05-channel-and-finance-deepening/041-payment-service-governance/) —— 🟢 **Approved（2026-09-26 负责人裁决批准立项，H-041-1~3 已裁决）**：payment-service 全域重构治理——Payment/Channel 对称四层（`channelgateway`→`channel` 正名、`posting` 并入 `payment`）、`ChannelGateway` 与 `PaymentResultPort` 双端口边界、渠道插件按供应方归档、一次性替换仓内 HTTP API、仅开发/测试环境清库重建 Schema、ArchUnit 强制边界与完整资金验收。⚠️ **ADR-0084 已起草但仍为 Proposed（待负责人 Accept）⇒ 当前不得动代码**；已完成 T01（ADR 草案 + 双向登记）、T02（四件套 + 实测回归基线 1086/0F/0E）、T03（[migration-map.md](stage-05-channel-and-finance-deepening/041-payment-service-governance/migration-map.md)）。**Supersedes ADR-0072 §6「分层 ≠ 拆事务」**；**须同步 ADR-0083 排除路径补 `/callbacks/**`**。
