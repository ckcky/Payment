...
- 端到端验证靠人肉按 `docs/specs/stage-01-core-mvp/001-core-business-model/quickstart.md` 手敲 curl，没有可重复执行、带断言的脚本；
...
## 2. 现状核实（基于真实代码，修正已归档的 `next-stage-design.md` 的三处偏差）

> `docs/archive/design/2026-09-19-next-stage-011-014/next-stage-design.md` 是 Draft，其中 §4.3 列出的两处"对账前置缺陷"经核对与当前代码不符。本 Spec 以**代码事实**为准，并在 §9 提出修订。
...
- `012-entry-idempotency` / `013-inventory-reservation` / `014-seckill-and-cache` 为**独立 Feature**（见 `docs/archive/design/2026-09-19-next-stage-011-014/next-stage-design.md`）：本 Spec 仅负责演示编排，不实现其领域逻辑。
