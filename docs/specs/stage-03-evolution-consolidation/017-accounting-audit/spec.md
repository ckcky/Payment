...
> **Status**: Implemented — 2026-09-06 负责人拍板全部决策点（见 [ADR-0065](../../../adr/0065-accounting-audit-suspense-adjustment.md)）；**2026-09-07 已实现并合并 master（63f73d1），450 测试全绿，live 冒烟闭环通过** <!-- Draft | In Review | Approved | In Development | Implemented | Deprecated | Superseded | Not Implemented -->
...
| # | 缺口 | 代码 / 文档证据 | 影响 |
|---|---|---|---|
| **G1** | **账证核对完全没有**（覆盖率无校验） | `grep -rn "ledger" reconciliation-service/src/main/java` **零命中**；`docs/specs/stage-01-core-mvp/004-ledger/spec.md:117` SC-005 明写「覆盖率 100%，**由 reconciliation 校验**」 | 业务已确认但账本漏记（记账 RPC 失败 / 重试耗尽）**无人发现**；反之账本孤儿分录也无人发现 |
