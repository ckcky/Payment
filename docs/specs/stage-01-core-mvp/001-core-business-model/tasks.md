...
**Input**: `docs/specs/stage-01-core-mvp/001-core-business-model/` 下的 plan.md、spec.md、research.md、data-model.md、contracts/ 和 quickstart.md
...
- [X] T076 [P] 更新 `docs/specs/stage-01-core-mvp/001-core-business-model/quickstart.md`，补充实际启动命令、验证命令和预期结果。
- [X] T077 运行 `mvnw verify`、Compose 验证和 quickstart 全链路 RPC 回归，记录 `docs/specs/stage-01-core-mvp/001-core-business-model/` 下的 MVP 验收结果；确认 Settlement 只生成模拟结算结果，不执行真实出款。
- [X] T078 运行 `/review` 和支付相关 `/payment-review`，检查 `docs/specs/stage-01-core-mvp/001-core-business-model/plan.md` 与实现结果，确认没有绕过模块边界、状态机、幂等或 Ledger 约束。
