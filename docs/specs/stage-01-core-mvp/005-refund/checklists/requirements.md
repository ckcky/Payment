...
- [x] CHK005 三项已核实缺口中 **G2 / G3 已补齐**（US2/US3 + Phase 4~5）；⛔ **G1 部分退款经裁决不做**（ADR-0016 Rejected，2026-08-30），US1/Phase 3 整体移除
...
- [x] CHK012 契约与 data-model 一致：⛔ ~~`refundedAmountMinor` 三处口径~~ → 已统一为**单一金额口径 `amountMinor`**（响应/请求不再传实际退款额，记账取 `amountMinor`）
...
- [x] CHK017 已核实 `PARTIALLY_SUCCEEDED` **保持不可达**（`RefundApplicationService` 只处理三态；ADR-0016 裁决不做，枚举保留但无调用方）
...
## 未决项（✅ 已于 2026-08-30 由负责人裁决，不再阻塞）

- [x] CHK021 ADR-0016（部分退款模型）→ ❌ **Rejected：不做**（Constitution §8.3 / §8.8）。已实现部分**全部回退**（2026-08-31）；
