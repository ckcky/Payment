...
- **SC-005**: 账本对所有已确认资金事实的覆盖率 100%（无「业务已确认但账本无分录」的孤儿事实，由 reconciliation 校验）。**已兑现**（2026-09-07，spec 017 A1 账证核对上线：`CertificateAuditor` 对已确认事实 100% 覆盖账本分录，MISSING_POSTING 差异为 0 即覆盖率 100%。）
  > ⚠️ **本条从未实现**——`reconciliation-service` 源码对 ledger 零引用，覆盖率校验不存在。已于 2026-09-06 立项 **spec 017 / ADR-0065**（账证核对 A1 + 科目勾稽 A2）兑现本条；落地后由 spec 017 tasks T082 回填本标注。
