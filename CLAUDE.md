...
| [docs/architecture/technical-solution.md](docs/architecture/technical-solution.md) | 总体技术方案（ADR 内联于正文对应章节；§7 项目计划与 §9 ADR 追溯索引已迁至 roadmap.md） | 涉及服务、端口、Schema、RPC 或部署 |
| [docs/architecture/systems/](docs/architecture/systems/) | 每服务系统设计文档（DDD 数据模型、API 契约、流程链路、存储缓存、部署拓扑） | 深入某一服务实现细节 |
| [docs/architecture/roadmap.md](docs/architecture/roadmap.md) | 项目阶段、当前状态和下一 Feature（含 §项目计划与资源、§附录 A ADR 追溯索引） | 开始或完成一个 Feature 前 |
| [docs/operations/runbook.md](docs/operations/runbook.md) | 运行手册：服务清单/端口、启动顺序、数据库、环境变量、常见故障与处置、回滚（同目录另有 `code-debt-backlog.md`、`split-proposal-template.md`） | 排障 / 运维 / 扩容拆分前 |
