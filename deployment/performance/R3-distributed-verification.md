...
> 当前开发机 **无 Docker**（`docker: command not found`），本清单在具备 Docker 的环境（或 CI）执行。
...
#    就绪校验（脚本内含 Nacos health 等待；另可手动）：
curl -fsS -o /dev/null "http://127.0.0.1:8848/nacos/actuator/health"
