 ...
 * 健康检查因此<b>恒定报 DOWN</b>——即使 Redis 完全可用（实测 Lua 原子预扣与缓存读写均正常）。
 *
 * <p>后果很实际：健康检查把正常实例报成故障，既掩盖真实故障（运维看到 DOWN 会误判），
 * 也让「Redis 不可用」的告警失去信噪比。本实现改用 {@code PING} 判定连通性，
 * 完全不解析 {@code INFO}，在 Windows/Linux 上行为一致。
 *
 * <p>仅判断连通性，不采集 Redis 版本等元信息——那些属于监控指标而非存活判据。
 *
