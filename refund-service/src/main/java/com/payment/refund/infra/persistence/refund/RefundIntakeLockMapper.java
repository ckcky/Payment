 ...
 *
 * <p>{@code INSERT ... ON DUPLICATE KEY UPDATE payment_id = payment_id} 语义为「确保行存在并持有其行锁」：
 * 首次受理插入锁行，并发受理则阻塞于该行的唯一键冲突，直至持锁事务提交/回滚，从而串行化同一支付下的
 * 累计退款金额读改写，杜绝超退款竞态。</p>
 */
public interface RefundIntakeLockMapper {

    @Insert("INSERT INTO refund_intake_locks (payment_id) VALUES (#{paymentId}) " +
            "ON DUPLICATE KEY UPDATE payment_id = payment_id")
    int lockForIntake(@Param("paymentId") Long paymentId);
}
