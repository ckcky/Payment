...
import com.payment.payment.infra.persistence.attempt.AttemptExtraCodec;
 ...
 *
 * <p>{@code PaymentAttemptChannelModeTest} 已分别锁死<b>领域侧</b>的读/写语义；
 ...
 *       {@value PaymentAttempt#CHANNEL_MODE_KEY} 且取值合法——不依赖调用方自觉；</li>
 ...
 * </ol>
 */
class PaymentAttemptExtraJsonTest {
    ...
    void openAttemptStampsModeKey() {
        DyeContext.runWith(DyeMode.SANDBOX, () -> attempts.openPaymentAttempt(PAYMENT_NO, "ALIPAY", 100L, "CNY"));

        PaymentAttempt saved = attempts.findByPaymentNo(PAYMENT_NO).get(0);
        assertThat(saved.getExtra())
                .as("写入口必须盖章，键缺失意味着反向路径永远读不到模态")
                .containsKey(PaymentAttempt.CHANNEL_MODE_KEY);
        assertThat(saved.getExtra().get(PaymentAttempt.CHANNEL_MODE_KEY)).isIn("MOCK", "SANDBOX");
    ...
    void openAttemptWithoutDyeStampsMock() {
        DyeContext.clear();
        attempts.openPaymentAttempt(PAYMENT_NO, "MOCK", 100L, "CNY");

        PaymentAttempt saved = attempts.findByPaymentNo(PAYMENT_NO).get(0);
        assertThat(saved.getExtra().get(PaymentAttempt.CHANNEL_MODE_KEY)).isEqualTo("MOCK");
    ...
    void openRefundAttemptStampsModeKey() {
        ...
        PaymentAttempt saved = attempts.findByPaymentNo(PAYMENT_NO).get(0);
        assertThat(saved.getAttemptType()).isEqualTo(PaymentAttempt.TYPE_REFUND);
        assertThat(saved.getChannelMode()).isEqualTo(DyeMode.SANDBOX);
    }
    ...
    void nullExtraReadsAsMock() {
        ...
        PaymentAttempt legacy = PaymentAttempt.rehydrate(1L, PAYMENT_NO, "ALIPAY", 0,
                Instant.now(), null, "ch-old", PaymentAttemptStatus.ACCEPTED, null, null, 1,
                PaymentAttempt.TYPE_PAYMENT, 100L, "CNY", decoded);
    ...
    void brokenJsonReadsAsMock() {
        ...
        PaymentAttempt attempt = PaymentAttempt.rehydrate(1L, PAYMENT_NO, "ALIPAY", 0,
                Instant.now(), null, "ch-old", PaymentAttemptStatus.ACCEPTED, null, null, 1,
                PaymentAttempt.TYPE_PAYMENT, 100L, "CNY", decoded);
    ...
    void missingKeyReadsAsMock() {
        ...
        PaymentAttempt attempt = PaymentAttempt.rehydrate(1L, PAYMENT_NO, "ALIPAY", 0,
                Instant.now(), null, null, PaymentAttemptStatus.ACCEPTED, null, null, 1,
                PaymentAttempt.TYPE_PAYMENT, 100L, "CNY", decoded);
    ...
    void repeatedStampingIsIdempotent() {
        ...
        // 同一模态重复盖章（模拟重复入口调用）
        attempt.putExtra(PaymentAttempt.CHANNEL_MODE_KEY, DyeMode.SANDBOX.name());
        attempt.putExtra(PaymentAttempt.CHANNEL_MODE_KEY, DyeMode.SANDBOX.name());

        assertThat(attempt.getExtra().get(PaymentAttempt.CHANNEL_MODE_KEY)).isEqualTo("SANDBOX");
        assertThat(attempt.getChannelMode()).isEqualTo(DyeMode.SANDBOX);

        // 再经一次完整落库-重建往返，模态不丢
        String json = AttemptExtraCodec.encode(attempt.getExtra());
        PaymentAttempt reloaded = PaymentAttempt.rehydrate(attempt.getId(), PAYMENT_NO, "ALIPAY", 0,
                attempt.getRequestedAt(), null, null, PaymentAttemptStatus.ACCEPTED, null, null, 1,
                PaymentAttempt.TYPE_PAYMENT, 100L, "CNY", AttemptExtraCodec.decode(json));
    ...
    void fullRoundTripKeepsSandbox() {
        DyeContext.runWith(DyeMode.SANDBOX, () -> attempts.openPaymentAttempt(PAYMENT_NO, "ALIPAY", 100L, "CNY"));
        PaymentAttempt original = attempts.findByPaymentNo(PAYMENT_NO).get(0);
        ...
        PaymentAttempt reloaded = PaymentAttempt.rehydrate(original.getId(), PAYMENT_NO, "ALIPAY", 0,
                original.getRequestedAt(), null, null, PaymentAttemptStatus.ACCEPTED, null, null, 1,
                PaymentAttempt.TYPE_PAYMENT, 100L, "CNY", AttemptExtraCodec.decode(columnValue));
    ...
    void legacyRowIsNotBackfilled() {
        PaymentAttempt legacy = PaymentAttempt.rehydrate(1L, PAYMENT_NO, "MOCK", 0,
                Instant.now(), null, null, PaymentAttemptStatus.SUCCEEDED, null, null, 1,
