...
/**
 * order-service 的 Feign 客户端：支付成功回写订单/交易状态 + 退款终态收口（spec 019）。
...
public interface OrderFeignClient extends OrderGateway {
    ...
    /** 退款终态收口（spec 019 / ADR-0067）：TXRF+PMRF 双号，order 按 TXRF 寻址幂等吸收。 */
    @PostMapping("/internal/orders/on-refund-result")
    @Override
    void notifyRefundResult(@RequestBody RefundResultNotification notification);
}
