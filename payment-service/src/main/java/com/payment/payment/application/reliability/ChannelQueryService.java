...
public class ChannelQueryService {
     ...
     * 内部包装为「单通道门面」——既有测试零改动。
     *
     * <p>attempt 仓储取不到时用「恒返回该单通道」的退化门面：既有单测不落 attempt 行，
    ...
    public ChannelQueryService(PaymentRepository paymentRepository,
        ...
        this.fallbackChannelCode = singleChannel.channelCode() == null
                ? "MOCK" : singleChannel.channelCode().toUpperCase();
    }

    /** 兼容路径的渠道兜底码（仅当 attempt 仓储为 null 时使用，即既有单测场景）。 */
    ...
    /**
     * 反向路径的调用目标（spec 030 / FR-272）：<b>渠道码</b> + 该 attempt 记录的模态 + 渠道交易号。
     *
     * <p>三者<b>必须同源</b>——都取自同一条 attempt 行。分头取会出现「用 A 渠道的码、
     * 配 B 渠道的模态、查 C 渠道的交易号」这类张冠李戴。</p>
     *
     * <p>spec 037 / T4：承载 <b>渠道码字符串</b>而非渠道实现——实现由门面在调用时解析
     * （{@link ChannelGateway#query}），本类因此不持有任何渠道类型。</p>
     */
    private record RecordedTarget(String channelCode, DyeMode mode, String channelReference) {
    }
    ...
    private RecordedTarget resolveRecordedTarget(Payment payment) {
        ...
        // spec 037 / T4：先验校验（INV-6）。解析本身由门面在 query 时做，但「可不可以解析」
        // 必须在此刻答完——调用方紧接着就要 recordQueryAttempt()，脏数据不得消耗查询次数。
