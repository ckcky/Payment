package com.payment.reconciliation.posting.infra;
...
import com.payment.reconciliation.posting.application.PostingReplayer;
...
/**
 * 台账补投分派器（spec 034 §12.1 / plan §2.3，reconciliation 落点）：把台账行的原始载荷
 * 原样重发到 ledger-service。
 *
 * <p><b>载荷零改写</b>：payload_json 与首次请求逐字节一致（登记时序列化原文），重放反序列化后
 * 原样重发——不重算派生键、不重构请求，杜绝二次漂移（plan §2.1）。</p>
 *
 * <p><b>幂等闸门在目的地</b>：Ledger 按 {@code ADJUSTMENT:{adjustNo}} 派生键吸收重复
...
public class PostingReplayDispatcher implements PostingReplayer {
    ...
    public PostingReplayDispatcher(PostingPendingRecorder recorder, LedgerAuditFeignClient ledgerClient) {
    ...
    public boolean replay(PendingPosting posting) {
        ...
        return switch (posting.getEventType()) {
            case PostingEventTypes.ADJUSTMENT -> {
                ...
                log.info("调账记账补投受理 adjustNo={} postingId={}",
